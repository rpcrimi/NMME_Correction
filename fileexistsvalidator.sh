...
do
	...
	python $fileexistsvalidator -s /datazone/nmme/output1/${institutions[$i]}/ -m ${models[$i]} -f 3hr -r atmos -d ${years[$i]} -l $workingdir/cron_logfiles/${models[$i]}/logs_$DD/3hr/ -u
	python $fileexistsvalidator -s /datazone/nmme/output1/${institutions[$i]}/ -m ${models[$i]} -f 6hr -r atmos -d ${years[$i]} -l $workingdir/cron_logfiles/${models[$i]}/logs_$DD/6hr/ -u
	python $fileexistsvalidator -s /datazone/nmme/output1/${institutions[$i]}/ -m ${models[$i]} -f day -r atmos -d ${years[$i]} -l $workingdir/cron_logfiles/${models[$i]}/logs_$DD/day/ -u
	python $fileexistsvalidator -s /datazone/nmme/output1/${institutions[$i]}/ -m ${models[$i]} -f mon -r ocean -d ${years[$i]} -l $workingdir/cron_logfiles/${models[$i]}/logs_$DD/mon/ -u
done
